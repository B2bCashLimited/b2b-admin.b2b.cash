...
import { filter, map, switchMap, debounceTime, distinctUntilChanged, startWith } from 'rxjs/operators';
...
export class PlatformComponent implements OnInit {
  ...
  pageSize = 0;
  length = 0;
  displayedColumns: string[] = ['select', 'name', 'contacts', 'dateTime', 'companies', 'activities', 'status', 'employees'];
  ...
  private _getAdminListUsers(page = 1): Observable<any> {
    ...
    const query = {
      adminLevel: 0,
      person: 2, // 1 - физ.лицо, 2 - юр.лицо, 0 - все
      ...filters
    };
    this.loading = true;
  ...
  getEmployees(user) {
    ...
    const companiesIds = user.companies.map(company => +company.id);
    this._matDialog.open(EmployeesDialogComponent, {
      data: companiesIds
    });
  }
  ...
  selectedActivityNameChange(activity) {
